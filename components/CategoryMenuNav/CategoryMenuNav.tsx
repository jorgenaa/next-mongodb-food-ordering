...
const CategoryMenuNav: NextPage<Props> = ({ uniqueProducts, uniquePath }) => {
  ...
  return (
    <nav className={s.categoryMenuNav}>
      <div
        className={s['categoryMenuNav__toggleBtn-wrapper']}
      ...
      <div
        className={
          isOpen
            ? `${s['categoryMenuNav__inner']} ${s['categoryMenuNav__inner--open']}`
            : `${s['categoryMenuNav__inner']} ${s['categoryMenuNav__inner--close']}`
        }
      >
        {uniqueProducts.map((prod: any, index: any) => {
          ...
          return (
            <div key={prod.category_title}>
              <ActiveLink
                activeClassName={s['CategoryMenuNav__link--active']}
                href={`${uniquePath}[slug]`}
                as={`${uniquePath}${prod.slug}`}
              >
                <a
                  className={`
                            ...
                            ${
                                router.asPath === p
                                ? s['categoryMenuNav__link--active']
                                : ''
                            }
                        `}
                >
